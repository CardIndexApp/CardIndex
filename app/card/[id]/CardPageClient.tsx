                  ...
                  {isLoggedIn && (
                    <div className="ci-card-actions" style={{ display: 'flex', flexDirection: 'column', gap: 6, alignItems: 'flex-end', flexShrink: 0 }}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          setLiveData(null)
                          const grade = urlGrade ?? 'Raw'
                          fetch(`/api/cache?id=${encodeURIComponent(id)}&grade=${encodeURIComponent(grade)}`, { method: 'DELETE' })
                            .finally(() => fetchLiveData(true))
                        }}
                        style={{ padding: '8px 14px', borderRadius: 10, background: 'var(--surface2)', border: '1.5px solid var(--border2)', fontSize: 11, fontWeight: 600, color: liveLoading ? 'var(--ink3)' : 'var(--ink2)', cursor: liveLoading ? 'default' : 'pointer', width: '100%', opacity: liveLoading ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, boxSizing: 'border-box' }}
                      ...
                      <button
                        ...
                        style={{ padding: '8px 14px', borderRadius: 10, background: 'var(--surface2)', border: '1.5px solid var(--border2)', fontSize: 11, fontWeight: 600, color: 'var(--ink3)', cursor: 'pointer', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, boxSizing: 'border-box' }}
                    ...
                    onClick={async () => {
                      ...
                      try {
                        await fetch('/api/report', {
                          ...
                          body: JSON.stringify({
                            card_id:     id,
                            card_name:   card?.name ?? liveData?.card_name ?? '',
                            grade:       urlGrade ?? 'Raw',
                            report_type: reportType,
                            description: reportDesc.trim() || null,
                            page_url:    window.location.href,
              ...
              <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap' }}>
                ...
                <div className="ci-card-actions" style={{ display: 'flex', flexDirection: 'column', gap: 8, alignSelf: 'flex-start', flexShrink: 0 }}>
                  ...
                  <button
                    className="ci-no-print"
                    onClick={() => { setReportOpen(true); setReportDone(false); setReportType(null); setReportDesc('') }}
                    style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '9px 14px', borderRadius: 10, background: 'var(--surface2)', border: '1.5px solid var(--border2)', fontSize: 11, fontWeight: 600, color: 'var(--ink3)', cursor: 'pointer', transition: 'all 0.2s' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#ff6b6b'; e.currentTarget.style.color = '#ff6b6b' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border2)'; e.currentTarget.style.color = 'var(--ink3)' }}
                  >
                    <svg width="11" height="11" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
