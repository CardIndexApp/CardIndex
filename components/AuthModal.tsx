    ...
    >
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 400, borderRadius: 20, background: 'var(--surface)', border: '1px solid var(--border2)', padding: 32 }}>
        ...
        {success ? (
          ...
          <>
            {/* Google OAuth — hidden on forgot-password screen */}
            {tab !== 'forgot' && (
              <>
                <button
                  type="button"
                  ...
                  disabled={loading}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, width: '100%', padding: '11px 14px', borderRadius: 12, background: 'var(--bg)', border: '1.5px solid var(--border2)', color: 'var(--ink)', fontSize: 14, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', transition: 'border-color 0.15s', marginBottom: 4 }}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--ink3)', whiteSpace: 'nowrap' }}>or continue with email</span>
                  <div style={{ flex: 1, height: 1, background: 'var(--border2)' }} />
                </div>
              </>
            )}
